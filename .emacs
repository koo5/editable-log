...
(setq circe-network-options
      `(("Freenode"
         :nick "editable-dog"
         :sasl-username "editable-dog"
         :sasl-password ,editable-log-password-freenode
         :nickserv-password ,editable-log-password-freenode
         :tls false
         :use-tls false
         :channels ("#swig"  "#logicmoo" "#autonomic-dev" "#autonomic" "#webofneeds" "#eulergui" "#lemonparty" "#atlas-project" "#netention"   "##AGI" "#nars"  "##idni"  "#emacs-circe"  "#swig" "#marpa" "#proglangdesign"))

      ("w3"
         :nick "editable-log"
         :sasl-username "editable-log"
;         :sasl-password ,editable-log-password-w3c
;        :nickserv-password ,editable-log-password-
         :tls false
         :use-tls false
         :channels ("#n3-dev"))))
     ...
     (dolist (buffer (buffer-list))
      ...
      (if (eq major-mode 'circe-channel-mode) (progn
          (let ((fn 
                ...
                (if (string= circe-chat-target "#atlas-project") "~/logs/atlas-project.log.txt"
          	...
          	(if (string= circe-chat-target "#nars") "~/logs/nars.log.txt" 
          	(if (string= circe-chat-target "#swig") "~/logs/swig.log.txt" 
          	(if (string= circe-chat-target "#logicmoo") "~/logs/logicmoo.log.txt" 
          	(if (string= circe-chat-target "#n3-dev") "~/logs/n3-dev.log.txt" 
          	'f))))))))))
